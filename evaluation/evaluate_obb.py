#!/usr/bin/env python3
"""Evaluate oriented-box (OBB) visual grounding with Acc@0.5, Acc@0.7 and mRIoU.

Same pipeline as evaluate_hbb.py: refGeo metainfo -> batched ms-swift inference with the
training prompt -> JSON parse -> coordinate mapping -> IoU -> per-split metrics, a
``<split>_obb_predictions.jsonl``, a summary JSON and a markdown table, then the macro
average over splits. The differences are:
  * the GT is the four-vertex ``poly`` and the answer must be
    ``[{"oriented_bbox": [[x1, y1], [x2, y2], [x3, y3], [x4, y4]]}]``;
  * the last JSON fragment in the output wins, because CoT-style answers state a coarse
    horizontal box first and refine it into the oriented box;
  * the score is the polygon intersection-over-union computed with shapely, after
    sorting the vertices around the centroid (see ``calculate_rotated_iou``);
  * only the three splits with genuine oriented annotations are evaluated
    (GeoChat, VRSBench, AVVG).

The strictness is deliberate: only a valid four-vertex ``oriented_bbox`` scores at all.
A model that falls back to a horizontal box under the OBB prompt gets 0, which is what
makes the OBB comparison across models meaningful. Coordinate modes (``norm1000``,
``absolute``, ``resized_absolute``) and the zero-row policy for missing images and failed
batches are those of evaluate_hbb.py.
...
METRIC_KEYS = ("acc@0.5", "acc@0.7", "mRIoU")
SCRIPT_TITLE = "OBB Evaluation · Acc@0.5 / Acc@0.7 / mRIoU"
DEFAULT_TABLE_FILE = "table3_acc05_acc07_mriou.md"
SUMMARY_FILE = "evaluation_obb_acc05_acc07_mriou_summary.json"
DEFAULT_COORD_MODE = "norm1000"
COORD_MODES = ("norm1000", "absolute", "resized_absolute")
QWEN_IMAGE_FACTOR = 28
Polygon = None  # shapely is imported lazily by _ensure_shapely, so --help works without it

# The three splits with genuine oriented annotations: metainfo file, image sub-directory
# and a default batch size picked by image resolution (4000x2250 AVVG images run at 12).
DATASET_CONFIGS = {
    "geochat_test": {
...
def _ensure_shapely() -> None:
    """Import shapely on first use; rotated IoU cannot be computed without it."""
    global Polygon
    ...
    try:
        ...
        raise ImportError("OBB evaluation requires shapely: pip install shapely") from exc
    Polygon = _Polygon


def _order_points_clockwise(points: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
    """Sort vertices by polar angle around their centroid.

    The model may list the four corners in any order, and an unsorted order can describe
    a self-intersecting polygon; sorting yields a simple polygon for any convex box.
    """
...
def calculate_rotated_iou(poly1: List[List[float]], poly2: List[List[float]]) -> float:
    """Polygon IoU of two four-vertex boxes living in the same coordinate space.

    Both polygons are centroid-sorted first (see ``_order_points_clockwise``); an invalid
    polygon, an empty union or any exception scores 0 rather than aborting the run.
    """
    _ensure_shapely()
    try:
        if len(poly1) != 4 or len(poly2) != 4:
            return 0.0
        corners1 = _order_points_clockwise([(float(p[0]), float(p[1])) for p in poly1])
        corners2 = _order_points_clockwise([(float(p[0]), float(p[1])) for p in poly2])
...
def parse_obb_from_output(output: str) -> Optional[List[List[float]]]:
    """Extract the last schema-conforming OBB from the raw model text.

    Code fences are stripped, every ``[ {...} ]`` fragment is located with a regex and
    decoded with ``json.loads`` (never ``eval``), scanning from the last fragment
    backwards: a CoT-style answer states a coarse horizontal box first and the oriented
    box last. Only a list whose first item is a dict with a four-vertex ``oriented_bbox``
    counts; anything else returns None and is scored as 0.
    """
    try:
        output = output.replace("```json", "").replace("```", "").strip()
        for snippet in reversed(re.findall(r"\[[\s\S]*?\{[\s\S]*?\}[\s\S]*?\]", output)):
            try:
...
def get_obb_prompt(question: str) -> str:
    """The OBB prompt, byte-identical to the training data and to demo/inference.py (the
    tests assert this); changing a character shifts the model's output distribution."""
    return f"""Locate the instance that matches the description: [{question}]. Report oriented bbox coordinates in following JSON format:
```json
[
\t{{"oriented_bbox": [[x1, y1], [x2, y2], [x3, y3], [x4, y4]]}}
]
...
def _maybe_denormalize_obb(
    obb: List[List[float]], image_size: Tuple[int, int], coord_mode: str
) -> List[List[float]]:
    """Map model coordinates into original-image pixels before computing RIoU.
    ...
    if coord_mode == "absolute":
        return [[float(pt[0]), float(pt[1])] for pt in obb]
    if coord_mode == "resized_absolute":
        resized_width, resized_height = _qwen_resized_size(image_size)
        return [
            [float(pt[0]) / resized_width * width, float(pt[1]) / resized_height * height]
            for pt in obb
        ]
    return [[float(pt[0]) / 1000.0 * width, float(pt[1]) / 1000.0 * height] for pt in obb]
...
def _metric_title(metric_key: str) -> str:
    ...
    return {"acc@0.5": "Acc@0.5", "acc@0.7": "Acc@0.7", "mRIoU": "mRIoU"}[metric_key]
...
def _metrics_from_rious(rious: List[float]) -> Dict[str, float]:
    """Acc@0.5, Acc@0.7 and mRIoU from the per-sample rotated IoUs of one split."""
    total = len(rious)
    if total == 0:
        return {metric: 0.0 for metric in METRIC_KEYS}
    all_metrics = {
        "acc@0.5": sum(1 for x in rious if x >= 0.5) / total,
        "acc@0.7": sum(1 for x in rious if x >= 0.7) / total,
        "mRIoU": sum(rious) / total,
    }
...
def evaluate_dataset(
    ...
    ``output_dir`` is set, writes ``<split>_obb_predictions.jsonl`` with the raw output,
    the parsed polygon in original-image pixels, the GT and the rotated IoU of every sample.
    """
    ...
    print(f"Evaluating OBB: {config['description']}{progress}")
    print(f"Metadata: {test_file}")
    print(f"Images: {image_dir}")
    print("IoU: rotated polygon IoU")
    ...
    for item in test_data:
        ...
        if gt_poly is None or len(gt_poly) != 4:
            continue
        ...
        if not os.path.exists(image_path):
            predictions.append({
                ...
                "output": "",
                "pred_obb": None,
                "pred_obb_display": None,
                "gt_obb": gt_poly,
                "rotated_iou": 0,
                "rotated_iou_display": f"{0:.4f}",
    ...
    # training prompt verbatim. 512 new tokens covers a CoT answer with two JSON blocks.
    request_config = RequestConfig(max_tokens=512)
    failed = 0
    for i in tqdm(range(0, len(valid_items), batch_size), desc=f"Evaluate {dataset_name} OBB (batch={batch_size})"):
        batch_items = valid_items[i:i + batch_size]
        batch_requests = []
        for item in batch_items:
            prompt = get_obb_prompt(item["question"])
        ...
        for item, output, error in zip(batch_items, outputs, errors):
            pred_obb = parse_obb_from_output(output) if error is None else None
            if pred_obb is not None:
                pred_obb = _maybe_denormalize_obb(pred_obb, item["image_size"], coord_mode)
                rotated_iou = calculate_rotated_iou(pred_obb, item["gt_poly"])
            else:
                rotated_iou = 0.0
            record = {
                ...
                "output": output,
                "pred_obb": pred_obb,
                "pred_obb_display": [[round(x, 2) for x in pt] for pt in pred_obb] if pred_obb is not None else None,
                "gt_obb": item["gt_poly"],
                "rotated_iou": rotated_iou,
                "rotated_iou_display": f"{rotated_iou:.4f}",
            }
            ...
            rious.append(rotated_iou)
            total += 1

    metrics = _metrics_from_rious(rious)

    if output_dir:
        os.makedirs(output_dir, exist_ok=True)
        pred_file = os.path.join(output_dir, f"{dataset_name}_obb_predictions.jsonl")
    ...
    result = {
        ...
        "correct": int(sum(1 for x in rious if x >= (0.5 if "acc@0.5" in METRIC_KEYS else 0.7))),
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", type=str, default="eval_results/obb", help="Result directory")
